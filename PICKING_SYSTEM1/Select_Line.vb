...
LOOP_MAIN_OPEN:
        ...
        ElseIf Line_list_view.FocusedItem.Index >= 0 Then
            If Line_list_view.Items.Count > 0 Then
                ...
                If arr_LVL(index) <> "1" And (arr_com_flg(index) = "0" Or arr_com_flg(index) = "2") Then
                    btn_ok.Visible = True
                End If
                If arr_LVL(index) = "1" Then 'Or arr_com_flg(index) = "1"
                    btn_ok.Visible = False 'จริง'
                    'btn_ok.Visible = True 'ชั่วคราว'
                End If
            End If
        Else
    ...
    Public Function query_detail_location() As String
        Try
            ...
            'Dim check_val As String = "SELECT count(id) as C_id from sup_frith_in_out where item_cd = '" & code_part & "' and com_flg <> 1" 'จริง'
            Dim check_val As String = "SELECT count(id) as C_id from sup_frith_in_out where item_cd = '" & code_part & "'" 'ชั่วคราว'
            'MsgBox(check_val)
    ...
    Public Function GET_DATA_WEB_POST(ByVal code_line As String, ByVal code_part As String, ByVal code_wi As String)
        ...
        Dim strCommand As String = "SELECT sd.*,F_I_O.LT ,F_I_O.QTY_OF_LOT , F_I_O.PO  FROM ( SELECT ss_f.item_cd, ss_f.com_flg,ss_f.LOT_RECEIVE as LT , ss_f.qty as QTY_OF_LOT ,ss_f.PUCH_ODR_CD as PO  , SUBSTRING (LOT_RECEIVE, 6, 8) AS date_lot FROM sup_frith_in_out ss_f  ) F_I_O INNER JOIN sup_work_plan_supply_dev sd ON F_I_O.item_cd = sd.ITEM_CD WHERE sd.line_cd = '" & code_line & "' AND sd.item_cd = '" & code_part & "' AND sd.wi = '" & code_wi & "' AND ( ps_unit_numerator <> '' AND sd.location_part <> '' ) AND sd.WORK_ODR_DLV_DATE  = '" & Module1.date_now_database & "' ORDER BY sd.wi, F_I_O.date_lot DESC" 'ชัวคราว
        'Dim strCommand As String = "SELECT sd.*,F_I_O.LT ,F_I_O.QTY_OF_LOT , F_I_O.PO  FROM ( SELECT ss_f.item_cd, ss_f.com_flg,ss_f.LOT_RECEIVE as LT , ss_f.qty as QTY_OF_LOT ,ss_f.PUCH_ODR_CD as PO  , SUBSTRING (LOT_RECEIVE, 6, 8) AS date_lot FROM sup_frith_in_out ss_f WHERE ss_f.com_flg <> 1 ) F_I_O INNER JOIN sup_work_plan_supply_dev sd ON F_I_O.item_cd = sd.ITEM_CD WHERE sd.line_cd = '" & code_line & "' AND sd.item_cd = '" & code_part & "' AND sd.wi = '" & code_wi & "' AND ( ps_unit_numerator <> '' AND sd.location_part <> '' ) AND sd.pick_flg != 1 AND sd.WORK_ODR_DLV_DATE  = '" & Module1.date_now_database & "' ORDER BY sd.wi, F_I_O.date_lot DESC" 'จริง
    ...
    Public Sub GET_DATA_FW(ByVal code_line As String, ByVal code_part As String, ByVal code_wi As String)
        Dim strCommand As String = "SELECT item_cd, item_name , qty , location_part , wi ,MODEL , PICK_QTY  FROM sup_work_plan_supply_dev WHERE line_cd  = '" & code_line & "' AND item_cd = '" & code_part & "'AND wi  = '" & code_wi & "' AND (ps_unit_numerator <> '' AND location_part <> '')  AND WORK_ODR_DLV_DATE  = '" & Module1.date_now_database & "' ORDER BY wi ASC" 'ชั่วคราว'
        'Dim strCommand As String = "SELECT item_cd, item_name , qty , location_part , wi ,MODEL , PICK_QTY  FROM sup_work_plan_supply_dev WHERE line_cd  = '" & code_line & "' AND item_cd = '" & code_part & "'AND wi  = '" & code_wi & "' AND (ps_unit_numerator <> '' AND location_part <> '') AND pick_flg != 1 AND WORK_ODR_DLV_DATE  = '" & Module1.date_now_database & "' ORDER BY wi ASC" 'จริง'
...
NEXT_END_FW:
    ...
    Public Function get_data_qty_po_lot(ByVal code_line As String, ByVal code_part As String, ByVal code_wi As String)
        ...
        ' Dim strCommand As String = "SELECT sd.*,F_I_O.LT ,F_I_O.QTY_OF_LOT , F_I_O.PO  FROM ( SELECT ss_f.item_cd, ss_f.com_flg,ss_f.LOT_RECEIVE as LT , ss_f.qty as QTY_OF_LOT ,ss_f.PUCH_ODR_CD as PO  , SUBSTRING (LOT_RECEIVE, 6, 8) AS date_lot FROM sup_frith_in_out ss_f WHERE ss_f.com_flg <> 1 ) F_I_O INNER JOIN sup_work_plan_supply_dev sd ON F_I_O.item_cd = sd.ITEM_CD WHERE sd.line_cd = '" & code_line & "' AND sd.item_cd = '" & code_part & "' AND sd.wi = '" & code_wi & "' AND ( ps_unit_numerator <> '' AND sd.location_part <> '' ) AND sd.pick_flg != 1 AND sd.WORK_ODR_DLV_DATE  = '" & Module1.date_now_database & "' ORDER BY sd.wi, F_I_O.date_lot ASC" 'จริง
        Dim strCommand As String = "SELECT sd.*,F_I_O.LT ,F_I_O.QTY_OF_LOT , F_I_O.PO  FROM ( SELECT ss_f.item_cd, ss_f.com_flg,ss_f.LOT_RECEIVE as LT , ss_f.qty as QTY_OF_LOT ,ss_f.PUCH_ODR_CD as PO  , SUBSTRING (LOT_RECEIVE, 6, 8) AS date_lot FROM sup_frith_in_out ss_f  ) F_I_O INNER JOIN sup_work_plan_supply_dev sd ON F_I_O.item_cd = sd.ITEM_CD WHERE sd.line_cd = '" & code_line & "' AND sd.item_cd = '" & code_part & "' AND sd.wi = '" & code_wi & "' AND ( ps_unit_numerator <> '' AND sd.location_part <> '' ) AND sd.WORK_ODR_DLV_DATE  = '" & Module1.date_now_database & "' ORDER BY sd.wi, F_I_O.date_lot ASC" 'ชั่วคราว
        Dim command As SqlCommand = New SqlCommand(strCommand, myConn)
        reader = command.ExecuteReader()
        Dim number As Integer = 1
        Dim RESULT_QTY As Integer = Module1.check_QTY
