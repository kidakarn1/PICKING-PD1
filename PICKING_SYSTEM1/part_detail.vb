...
Public Class part_detail
    ...
    Private Sub part_detail_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Finally
            ...
            '--------------------------'
            show_number_remain.Visible = False 'ชั่วคราว'
            show_number_supply.Visible = False 'ชั่วคราว'
            want_to_tag.Visible = False
            'get_data_tetail() 'ชั่วคราว
            'check_qr_part()
        End Try
    End Sub
    Public Sub New()
...
go_pick_detail_number:
                            ...
                            Else
                                If show_number_supply.Text > req_qty And firstscan = "0" And number_remain > 0 Then
                                    ...
                                    'scan_qty.Visible = False  'ไม่เกี่ยว'
                                    comp_flg = "1"
                                    Panel7.Visible = True
                                ...
                                ElseIf req_qty = show_number_supply.Text And number_remain = 0 Then
                                    ...
                                    scan_seq_arrlist.Add(order_number & supp_seq)
                                    comp_flg = "1"
                                    'scan_qty.Visible = False 'ชั่วคราว'
                                    check_scan = 2
                                    'Panel7.Visible = True'ชั่วคราว'
                                    'alert_success.Visible = True'ชั่วคราว'
                                    'status_alert_image = "success"'ชั่วคราว'
                                    'text_box_success.Focus()'ชั่วคราว'
                                    GoTo exit_scan
                                    'เคสยิงสะสม
                                ElseIf show_number_supply.Text = req_qty And number_remain > 0 Then
                                    ...
                                    comp_flg = "1"
                                    'Panel7.Visible = True 'ชั่วคราว'
                                    'alert_success_remain.Visible = True 'ชั่วคราว'
                                    'status_alert_image = "success_remain" 'ชั่วคราว'
                                    'text_box_success.Focus() 'ชั่วคราว'
                                    GoTo exit_scan
                                Else
...
go_pick_detail_number_fw:
                            ...
                            Else
                                If show_number_supply.Text > req_qty And firstscan = "0" And number_remain > 0 Then
                                    ...
                                    check_text_box_qr_code()
                                    'scan_qty.Visible = False 'ชั่วคราว'
                                    'text_box_success.Visible = True 'ชั่วคราว'
                                    'text_box_success.Focus() 'ชั่วคราว'
                                    'Panel7.Visible = True 'ชั่วคราว'
                                    'alert_success_remain.Visible = True 'ชั่วคราว'
                                    'status_alert_image = "success_remain" 'ชั่วคราว'
...
remain_seq_FW:
    ...
    Private Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button2.Click
        ...
        Dim pinlen As UInt32 = CType(pin.Length, UInt32)
        Dim temporary = "temporary" 'ชั่วคราว'
        If temporary <> "temporary" Then 'ชั่วคราว ทำเพื่อไม่ให้มี tag print'
loop_check_open_printer:
        ...
        Try
            ...
            Do While reader.Read()
                ...
                ' MsgBox("4")
                If check_scan = 1 Then
                    total_pig_qty = CDbl(Val(reader("PICK_QTY").ToString)) + CDbl(Val(show_number_supply.Text))
            ...
            Loop
            reader.Close()
            ' MsgBox("check_scan2= " & check_scan)
            'MsgBox("total_pig_qty2 = " & total_pig_qty)
            If check_scan = 1 Then
                ' strCommand = "UPDATE sup_work_plan_supply_dev SET update_date = '" & date_now & "' , pick_flg = '2' , PICK_QTY = '" & total_pig_qty & "' ,  update_by = '" & emp_cd & "' , term_cd = '" & term_id & "'   WHERE wi  = '" & sel_where1 & "' AND item_cd = '" & sel_where2 & "'"   'จริง'
                ' MsgBox("strCommand = " & strCommand)
            ElseIf check_scan = 2 Then
                ' strCommand = "UPDATE sup_work_plan_supply_dev SET update_date = '" & date_now & "' , pick_flg = '1' , PICK_QTY = '" & total_pig_qty & "' , update_by = '" & emp_cd & "' , term_cd = '" & term_id & "'   WHERE wi  = '" & sel_where1 & "' AND item_cd = '" & sel_where2 & "'"   'จริง'
                'MsgBox("strCommand = " & strCommand)'ชั่วคราว'
            End If
            'MsgBox(strCommand)'ชั่วคราว'
            strCommand = "UPDATE sup_work_plan_supply_dev SET update_date = '" & date_now & "' , pick_flg = '1' , PICK_QTY = '0' , update_by = '" & emp_cd & "' , term_cd = '" & term_id & "'   WHERE wi  = '" & sel_where1 & "' AND item_cd = '" & sel_where2 & "'"   'ชั่วคราว'
            Dim command As SqlCommand = New SqlCommand(strCommand, myConn) 'ชั่วคราว'
            reader = command.ExecuteReader() 'ชั่วคราว'
            reader.Close() 'จริง'
        Catch ex As Exception
            MsgBox("Can not update into database")
        End Try
        ...
        Try
            ...
            If check_remain(scan_lot, updated_seq, F_item_cd) = 1 Then
                ...
                If REMAIN_ID <> "NO_DATA" Then
                    ...
                    If Len_length = 62 Then
                        ' WEB_POST_Cut_stock_frith_in_out(PO, F_item_cd, scan_qty, tag_readed, updated_seq, com_flg_table, tag_remain_qty) 'ชั่วคราว'
                    ElseIf Len_length = 103 Then
                        ...
                        'FW_Cut_stock_frith_in_out(PO, F_item_cd, scan_qty, tag_readed, updated_seq, com_flg_table, tag_remain_qty, scan_lot)'ชั่วคราว'
                    End If
                End If
            Else
...
re_check:
        ...
        Return True
    End Function
    Public Function TMP_get_qty_remain()
        Dim status As Integer = 0
        Dim get_qty As String = "NO_DATA"
        Try
            Dim strCommand123 As String = "SELECT tag_remain_qty as remain_qty  FROM sup_scan_pick_detail  where com_flg = '0' and tag_readed ='" & scan_qty.Text & "'"
            'MsgBox("check_remain_in_detail_test == >" & strCommand123)
            Dim command123 As SqlCommand = New SqlCommand(strCommand123, myConn)
            reader = command123.ExecuteReader() 'ติดบรรทัดนี้'
            Do While reader.Read
                get_qty = reader("remain_qty").ToString()
            Loop
            reader.Close()
            scan_qty.Text = ""
            scan_qty.Focus()
        Catch ex As Exception
            MsgBox("ERROR set faill  " & vbNewLine & ex.Message, 16, "Status")
        End Try
        Return get_qty
    End Function
